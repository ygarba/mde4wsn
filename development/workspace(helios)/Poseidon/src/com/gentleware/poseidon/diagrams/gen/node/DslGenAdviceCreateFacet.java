...
import com.gentleware.poseidon.diagrams.gen.node.DslGenAdviceCreatorGem;
...
public abstract class DslGenAdviceCreateFacet extends CustomBaseNodeCreateFacet {

	public static final UDimension DEFAULT_SIZE = new UDimension(
			10 * ZGridLayer.DEFAULT_GRID_SPACING,
			8 * ZGridLayer.DEFAULT_GRID_SPACING);

	public static final String DEFAULT_FIGURE_NAME = "Advice";
	public static final String SEMANTIC_ELEMENT = "com.gentleware.poseidon.model.wrapper.WrappedAdvice";

	protected DslGenAdviceCreatorGem creatorGem;

	public DslGenAdviceCreatorGem getCreatorGem() {
		return creatorGem;
	}

	public void setCreatorGem(DslGenAdviceCreatorGem creatorGem) {
	...
	public Object createFigure(Object subject, DiagramFacet diagram,
		...
		if (containerFacet != null) {
			containerFacet.getContentRoles()
					.add(DslGenElementRoles.INITIALNODE);

			containerFacet.getContentRoles().add(DslGenElementRoles.FINALNODE);

			containerFacet.getContentRoles().add(
					DslGenElementRoles.DECISIONNODE);

			containerFacet.getContentRoles().add(DslGenElementRoles.MERGENODE);

			containerFacet.getContentRoles().add(
					DslGenElementRoles.SENSINGACTION);

			containerFacet.getContentRoles().add(
					DslGenElementRoles.ACTUATINGACTION);

			containerFacet.getContentRoles().add(
					DslGenElementRoles.TIMINGACTION);

			containerFacet.getContentRoles().add(
					DslGenElementRoles.OPAQUEACTION);

			containerFacet.getContentRoles().add(
					DslGenElementRoles.UPDATEPOOLACTION);

			containerFacet.getContentRoles().add(
					DslGenElementRoles.GETNODEACTION);

			containerFacet.getContentRoles().add(
					DslGenElementRoles.GETDATAACTION);

			containerFacet.getContentRoles().add(
					DslGenElementRoles.REMOTETRIGGERACTION);
	...
	public EObject createSemanticElement(EObject owner,
		...
		DslGenSemanticElementFactory factory = DslGenSemanticElementFactory
				.getInstance();
		return factory.createAdviceSemanticElement(owner);
	}

	public String getFigureName() {
		return DslGenAdviceCreatorGem.NAME;
	}
	...
	public void initNodeRoles() {
		...
		roles.add(DslGenElementRoles.ADVICE);
	}
